...
namespace WebApiProxy.Server
{
    ...
    {
        readonly List<ModelDefinition> models
            = new List<ModelDefinition>();

        readonly HashSet<string> typesToIgnore = new HashSet<string>();
        ...
        {
            var res = genericTypeDefClass.Name;
            var index = res.IndexOf('`');
            if (index > -1)
                res = res.Substring(0, index);

            var args = genericTypeDefClass.GetGenericArguments();
        ...
        private void AddModelDefinition(Type classToDef)
        {
            ...
            {
                typesToIgnore.Add(classToDef.Name);
                return;
            }
            ...
            if (!typesToIgnore.Contains(classToDef.Name))
            {
                ...
                {
                    Name        = classToDef.Name,
                    Description = GetDescription(classToDef),
                    Type        = classToDef.IsEnum ? "enum" : "class"
                };
        ...
        private string GetConstantValue(FieldInfo constant) =>
            constant.GetRawConstantValue().ToString();
